// How compiler know the address of an array whenever you want to do action on an array. Thing is that they know the base address of an array. That's say the address of A[0] is 200 and there are 5 integer in this array. And each integer would take 2 Bytes. So that if you want to alter the number that saved in the A[3], the compiler will calculate the address of it, which is 200 + 3*2 = 206. 200 is the base address and 3 is index. and 2 is the Bytes that integer takes. If it is formula, it would be like Lo + i * w
...
// Actually for computer, they don't think that it is a 2D array, esepcially in terms of how they keep the value. They keep the value and store the value of the array just like 1D array. Raw by Raw. Add(A[1][2]) = 200 + [1*4 + 2] *2
